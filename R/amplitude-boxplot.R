...
AmplitudeBoxplot <- function(warp_median,
                             alpha = 0.05,
                             ka = 1,
                             showplot = TRUE) {
  fn <- warp_median$fn
  ...
  if (warp_median$method != 'median')
    cli::cli_abort(c(
      'The {.fn amplitude_boxplot} requires that alignment be performed on the median.',
      "x" = 'You have supplied an object of class {.cls class(warp_median)} with {.code method == "mean"}.',
      'Please rerun {.fn time_warping} with {.code method == "median"}.'
    ))
