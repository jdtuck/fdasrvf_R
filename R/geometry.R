...
findkarcherinv <- function(warps, times, round = F){
  ...
  check <- 1
  iter <- 0
  while (check > 0.01 && iter < 1000) {
    for (i in 1:n){
      ip <- min(max(sum(mupsi*psi.m[,i]/(m-1)), -1), 1)
      theta <- acos(ip)
      if (theta < 1e-10)
        v.m[,i] <- 0
      else
    ...
    # step along the geodesic towards the Karcher mean
    if (check > 0)
      mupsi <- cos(0.01*check)*mupsi + sin(0.01*check)*vbar/check
    iter <- iter + 1
  }
  karcher.s <- 1+c(0,cumsum(mupsi^2)*times)
  if(round){
