...
f.exp1inv <- function(psi){
  ...
  if( (inner<1.001) && (inner >=1) ){inner=1}
  if( (inner<=-1) && (inner>-1.001) ){inner=-1}
  if( (inner<(-1)) || (inner>1) ){print(paste("exp1inv: can't calculate the acos of:",inner))}
  
  theta=acos(inner)
  yy=theta/sin(theta)*(y-rep(inner,length(y)))
  
  if(theta==0){yy=rep(0,length(x))}
  
  return( list(x=x, y=yy) ) 
}

# function for calculating the next MCMC sample given current state
f.updateg.pw <- function(g.coef.curr,g.basis,var1.curr,q1,q2,SSE.curr=0,propose.g.coef){
  g.coef.prop = propose.g.coef(g.coef.curr=g.coef.curr)
  while( min( f.exp1(f.basistofunction(f.domain=g.basis$x,coef=g.coef.prop$prop,basis=g.basis))$y)<0 ){
    g.coef.prop = propose.g.coef(g.coef.curr=g.coef.curr)
  }
  
  if(SSE.curr==0){
    SSE.curr=f.SSEg.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.curr,basis=g.basis),
                       q1=q1,q2=q2)
    }
  
  SSE.prop=f.SSEg.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.prop$prop,basis=g.basis),
                     q1=q1,q2=q2)
  
  logl.curr=f.logl.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.curr,basis=g.basis),
                      var1=var1.curr, q1=q1, q2=q2, SSEg=SSE.curr)
  logl.prop=f.logl.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.prop$prop,basis=g.basis),
                      var1=var1.curr, q1=q1, q2=q2, SSEg=SSE.prop)
  
  ratio=min(1, exp(logl.prop - logl.curr) )  #prob to accept the proposal
  u=runif(1)
  if(u<=ratio){return( list(g.coef=g.coef.prop$prop,logl=logl.prop,SSE=SSE.prop, accept = TRUE, zpcnInd = g.coef.prop$ind ))}
  if(u>ratio){return( list(g.coef=g.coef.curr,logl=logl.curr,SSE=SSE.curr,  accept = FALSE, zpcnInd = g.coef.prop$ind))} 
}
...
# returns: $y==f(at), $x==at
################################################################################
f.predictfunction <- function(f, at, deriv=0, method="linear"){
  if(method=="cubic"){
    result=predict(smooth.spline(f$x,f$y,all.knots=F,nknots=(length(f$x)-10)),at,deriv=deriv)
  ...
  if(method=="linear"){
    ...
    if(deriv==1){
      ...
      result=list(x=at,y=(diffy2+diffy1)/(diffx2+diffx1))
...
#     $x: f.domain
################################################################################
basis.fourier <- function(f.domain=0,numBasis=1,fourier.p=1){
  result=matrix(NA,nrow=length(f.domain),ncol=2*numBasis)
  for(i in 1:(2*numBasis)){
    j=ceiling(i/2)
    if((i%%2)==1){result[,i]=sqrt(2)*sin(2*j*pi*f.domain/fourier.p)}
    if((i%%2)==0){result[,i]=sqrt(2)*cos(2*j*pi*f.domain/fourier.p)}
  }
...
################################################################################
f.basistofunction <- function(f.domain, coefconst=0, coef, basis, plotf=F){
...
f.exppsiinv <- function(psi,psi2){
  ...
  if( (inner<(-1)) || (inner>1) ){print(paste("exppsiinv: can't calculate the acos of:",inner))}
  theta=acos(inner)
...
############################################################################################
f.psimean <- function(x,y,e1=0.001,e2=0.3,method='ext'){
  if(method=='int'){
    ...
    while(  f.L2norm(list(x=x,y=vbar)) >= e1 ){
      for(i in 1:p){
        #i=1
        vM[,i]=f.exppsiinv(psi=list(x=x,y=result),psi2=list(x=x,y=y[,i]))$y
        #dM[i]=acos(trapz(f.domain, result*psiM[,i]))
      }
      #energy=c(energy,sum(dM^2))
      vbar=rowMeans(vM)
      #update
      result=f.exppsi(psi=list(x=x,y=result),g=list(x=x,y=e2*vbar))$y
      j=j+1
...
pair_align_functions_expomap <- function(f1, f2, timet, iter = 2e4,
  burnin = min(5e3, iter/2), alpha0 = 0.1, beta0 = 0.1,
  zpcn = list(betas = c(0.5, 0.05, 0.005, 0.0001), probs = c(0.1, 0.1, 0.7, 0.1)),
  propvar = 1, init.coef = rep(0, 2*10), npoints = 200, extrainfo = FALSE) {
  ...
  pw.sim.global.domain.par <- seq(from = 0, to = 1, length.out = numSimPoints)
  g.basis <- basis.fourier(f.domain = pw.sim.global.domain.par,
    numBasis = pw.sim.global.Mg, fourier.p = 1)
  sigma1.ini <- 1
    ...
    for(i in 1:length(pCN.beta)){
      if(z <= probm[i+1] && z > probm[i]){
        g.coef.new=rnorm(pw.sim.global.Mg*2,
                         sd=pw.sim.global.sigma.g/rep(c(1:pw.sim.global.Mg),each=2))
        result<- list(prop = sqrt(1-pCN.beta[i]^2)*g.coef.curr+pCN.beta[i]*g.coef.new, ind = i)
      }
    }
    return (result) 
  }
  ...
  if( min( f.exp1(f.basistofunction(f.domain=g.basis$x,coef=g.coef.ini,
    basis=g.basis))$y)<0 ){
  ...
  SSE.curr=f.SSEg.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.ini,
    basis=g.basis),q1=q1,q2=q2)
  logl.curr=f.logl.pw(
    g=f.basistofunction(
      f.domain=g.basis$x,
      coef=g.coef.ini,
  ...
  for(m in 2:iter){
    #update g
    a=f.updateg.pw(g.coef.curr=g.coef.curr,
                         g.basis=g.basis,
    ...
    logl.curr=f.logl.pw(g=f.basistofunction(f.domain=g.basis$x,coef=g.coef.curr,basis=g.basis),
                        var1=sigma1.curr^2, q1=q1, q2=q2, SSEg=SSE.curr)
  ...
  pw.sim.est.psi.matrix <- matrix(NA, nrow = length(pw.sim.global.domain.par), ncol = length(valid.index))
  ...
  result.posterior.psi.simDomain <- f.psimean(
  ...
  result.posterior.psi <- with(result.posterior.psi.simDomain,
    approx(x = x, y = y, xout = f1$x, rule = 2))
  ...
  # warped f2
  f2.warped <- with(result.posterior.gamma, warp_f_gamma(f = f2$y, time = x,
    gamma = y))
  ...
  # return object
  retVal <- list(f2_warped = f2.warped,
