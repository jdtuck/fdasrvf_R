#' @export
plot.phbox <- function(out){
  median_x <- out$median_x
  maxx <- out$maxx
  minn <- out$minn
  Q1 <- out$Q1
  Q1a <- out$Q1a
  ...
  d1<-sqrt(trapz(time,(out$psi_median-out$Q1_psi)^2))
  d1a<-sqrt(trapz(time,(out$Q1_psi-out$Q1a_psi)^2))
  dl<-sqrt(trapz(time,(out$Q1a_psi-out$min_psi)^2))
  d3<-sqrt(trapz(time,(out$psi_median-out$Q3_psi)^2))
  d3a<-sqrt(trapz(time,(out$Q3_psi-out$Q3a_psi)^2))
  du<-sqrt(trapz(time,(out$Q3a_psi-out$max_psi)^2))
