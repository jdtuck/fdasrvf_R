...
plot.phbox <- function(x, ...){
  ...
  d1<-acos(trapz(time,(x$psi_median-x$Q1_psi)^2))
  d1a<-acos(trapz(time,(x$Q1_psi-x$Q1a_psi)^2))
  dl<-acos(trapz(time,(x$Q1a_psi-x$min_psi)^2))
  d3<-acos(trapz(time,(x$psi_median-x$Q3_psi)^2))
  d3a<-acos(trapz(time,(x$Q3_psi-x$Q3a_psi)^2))
  du<-acos(trapz(time,(x$Q3a_psi-x$max_psi)^2))
