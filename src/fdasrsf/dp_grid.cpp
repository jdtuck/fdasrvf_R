    ...
    int aidx, int cidx, double lam, int pen)
{
  ...
  Q1idx = aidx; /*dp_lookup( T1, nsamps1, a );*/
        ...
        /* The penalty depends only on the slope of the edge, so evaluate it
         * once here.  DP_PEN_NONE leaves pen_term at 0, i.e. no penalty.
         * These match the penalties of CostFn2() in DP.cpp. */
        switch ( pen )
        {
